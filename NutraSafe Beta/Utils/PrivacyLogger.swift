...
/*
 ...
 logDebug("App launched successfully")
 logInfo("Loading user settings")
 logWarning("Network connection slow")
 logError("Failed to save data", error: someError)

 EXAMPLE 2: Category-specific logging

 logDebug("User authenticated", subsystem: .auth, category: .authentication)
 logInfo("Syncing food entries", subsystem: .firebase, category: .dataSync)

 EXAMPLE 3: Private data logging (redacted in production)

 PrivacyLogger.debugPrivate("User signed in", privateData: user.email)
 PrivacyLogger.authEvent("Sign In", userId: user.id, email: user.email)

 EXAMPLE 4: Network logging

 PrivacyLogger.networkRequest(method: "GET", url: "https://api.example.com/foods?query=apple", statusCode: 200)
 // Logs: "GET https://api.example.com/foods - Status: 200" (query redacted)

 EXAMPLE 5: Cache logging

 PrivacyLogger.cacheEvent("HIT", key: "food_entries_2025-12-14")
 PrivacyLogger.cacheEvent("MISS", key: "user_settings")

 EXAMPLE 6: Security events

 PrivacyLogger.securityEvent("Unauthorized access attempt detected", severity: .error)
 PrivacyLogger.securityEvent("Admin access granted", severity: .info)

 EXAMPLE 7: Validation logging

 PrivacyLogger.validationFailure(field: "calories", value: -100, error: ValidationError.invalidCalories(value: -100))
 */
