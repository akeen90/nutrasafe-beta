...
/*
 EXAMPLE 1: Update user settings with version check (prevents concurrent conflicts)

 let settingsRef = db.collection("users").document(userId)
                     .collection("settings").document("preferences")

 try await db.updateWithVersionCheck(
 ...
 EXAMPLE 2: Atomic batch - save diary entry and update stats together
 ...
 let statsRef = db.collection("users").document(userId)
                  .collection("stats").document("daily")

 try await db.performAtomicBatch(operations: [
     (diaryRef, entryData),
     (statsRef, ["totalCalories": FieldValue.increment(500)])
 ])

 EXAMPLE 3: Read-modify-write for complex updates

 let result = try await db.readModifyWrite(documentRef: settingsRef) { currentData in
     var updated = currentData
     let currentCalories = currentData["caloricGoal"] as? Int ?? 2000
     updated["caloricGoal"] = max(1200, currentCalories + 100)
     return updated
 }

 EXAMPLE 4: Conditional update (only update if condition met)

 let updated = try await FirestoreTransactionHelper.conditionalUpdate(
     documentRef: settingsRef,
     condition: { data in
         // Only update if user hasn't set custom goal
         return data["hasCustomGoal"] as? Bool != true
     },
     updateData: ["caloricGoal": 2000],
     db: db
 )
 */
