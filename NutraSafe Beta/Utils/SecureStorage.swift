...
/*
 EXAMPLE 1: Store and retrieve user allergens

 // Store
 let allergens: [Allergen] = [.dairy, .eggs, .peanuts]
 try SecureStorage.set(allergens, forKey: .allergens)

 // Retrieve
 if let allergens: [Allergen] = SecureStorage.getOptional(forKey: .allergens) {
     print("User allergens: \(allergens)")
 }

 EXAMPLE 2: Store custom Codable type
 ...
 try SecureStorage.set(profile, forKey: "health_profile")

 EXAMPLE 3: Migrate from UserDefaults

 SecureStorage.migrateFromUserDefaults(mappings: [
     "user_allergens": .allergens,
     "user_weight": .currentWeight,
     "user_height": .height
 ])

 EXAMPLE 4: Delete sensitive data

 SecureStorage.delete(forKey: .allergens)
 SecureStorage.delete(forKey: .medicalConditions)
