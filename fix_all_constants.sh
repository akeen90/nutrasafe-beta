...
# Script to replace ALL constants in ALL Swift files
echo "Starting comprehensive constant replacement..."

# Find all Swift files in the entire project
PROJECT_DIR="/Users/aaronkeen/Documents/My Apps/NutraSafe Beta/NutraSafe Beta"

# Replace AtomicConstants
find "$PROJECT_DIR" -name "*.swift" -type f -exec sed -i '' \
  ...
  -e 's/AtomicConstants\.standardAnimationDuration/0.3/g' \
  -e 's/AtomicConstants\.mediumOpacity/0.6/g' \
  -e 's/AtomicConstants\.touchTargetSize/44/g' \
  -e 's/AtomicConstants\.pressedScale/0.95/g' \
  -e 's/AtomicConstants\.standardSpringDamping/0.7/g' \
  -e 's/AtomicConstants\.standardSpringResponse/0.5/g' \
  {} \;
...
echo "Comprehensive constant replacement complete!"
echo "Files processed:"
find "$PROJECT_DIR" -name "*.swift" -type f | wc -l