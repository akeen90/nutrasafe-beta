...
exports.recognizeFood = functions
    ...
    try {
        ...
        for (const identified of identifiedFoods) {
            ...
            const aiValidation = validateNutritionData(identified.estimatedCaloriesPer100g, identified.estimatedProteinPer100g, identified.estimatedCarbsPer100g, identified.estimatedFatPer100g, `AI estimate: ${identified.name}`);
 ...
 */
function isNonFoodProduct(name, brand) {
 ...
 */
function validateNutritionData(caloriesPer100g, proteinPer100g, carbsPer100g, fatPer100g, foodName) {
    const issues = [];
    // Check for negative values
    if (caloriesPer100g < 0)
        issues.push(`Negative calories: ${caloriesPer100g}`);
    if (proteinPer100g < 0)
        issues.push(`Negative protein: ${proteinPer100g}`);
    if (carbsPer100g < 0)
        issues.push(`Negative carbs: ${carbsPer100g}`);
    if (fatPer100g < 0)
        issues.push(`Negative fat: ${fatPer100g}`);
    ...
    if (proteinPer100g > 100)
        issues.push(`Protein too high: ${proteinPer100g}g/100g (max ~90g for pure protein)`);
    if (carbsPer100g > 100)
        issues.push(`Carbs too high: ${carbsPer100g}g/100g`);
    if (fatPer100g > 100)
        issues.push(`Fat too high: ${fatPer100g}g/100g`);
    ...
    const hasCriticalIssue = issues.some(issue => issue.includes('Negative') ||
...
async function searchDatabaseForFood(client, food) {
    ...
    const getSourceTier = (indexName) => {
        if (indexName === 'tesco_products')
            return 0; // Tier 1: Official supermarket data
        if (indexName === 'uk_foods_cleaned')
            return 1; // Tier 2: UK verified data
