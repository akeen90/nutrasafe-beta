...
// Interface for comprehensive UK additives data
interface AdditiveInfo {
  code: string;
  name: string;
  category: string;
  permitted_GB: boolean;
  permitted_NI: boolean;
  permitted_EU: boolean;
  status_notes?: string;
  child_warning: boolean;
  PKU_warning: boolean;
  polyols_warning: boolean;
  sulphites_allergen_label: boolean;
  origin: string;
  consumer_guide: string; // Complete health-conscious parent guide with bold headers
  effects_verdict: 'neutral' | 'caution' | 'avoid';
  synonyms: string[];
  matches: string[];
  ins_number?: string;
  sources: Array<{
...
// Interface for processing grade rules
interface ProcessingGradeRules {
  version: string;
  score_to_grade: Array<{
    min?: number;
    max?: number;
    grade: string;
    label: string;
  ...
  category_weights_notes: {
    [key: string]: {
      ...
      us_color_names?: string[];
      multiple_color_penalty?: number;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (columns.length >= 18) {
        ...
        let effects_verdict: 'neutral' | 'caution' | 'avoid' = 'neutral';
        const grade = columns[10].trim();
        if (grade === 'Avoid') effects_verdict = 'avoid';
        else if (grade === 'Limit') effects_verdict = 'caution';
        else if (grade === 'Generally OK') effects_verdict = 'neutral';
...
function loadEssentialAdditives(filePath: string) {
  ...
  for (const [code, essentialAdditive] of Object.entries(essentialData)) {
    const essential = essentialAdditive as any;
    
    // Convert grade to effects_verdict
    let effects_verdict: 'neutral' | 'caution' | 'avoid' = 'neutral';
    if (essential.grade === 'Avoid') effects_verdict = 'avoid';
    else if (essential.grade === 'Limit') effects_verdict = 'caution';
    else if (essential.grade === 'Generally OK') effects_verdict = 'neutral';
...
function addEnhancedHiddenAdditives() {
  ...
  const hiddenAdditives: { [key: string]: AdditiveInfo } = {
...
Must be declared on labels but often people don't realize what it is`,
      effects_verdict: 'neutral' as const,
...
Still legal but many manufacturers voluntarily removing due to health concerns`,
      effects_verdict: 'caution' as const,
...
export const analyzeAdditivesEnhanced = functions.https.onRequest(async (req, res) => {
...
interface AdditiveAnalysisResult {
  ...
  pkuWarnings: string[];
  sulphiteWarnings: string[];
  polyolWarnings: string[];
...
function analyzeIngredientsForAdditives(ingredients: string): AdditiveAnalysisResult {
  const text = ingredients.toLowerCase();
  const detectedAdditives: AdditiveInfo[] = [];
  ...
  for (const [, additive] of Object.entries(COMPREHENSIVE_ADDITIVES_DB)) {
    if (foundCodes.has(additive.code)) continue; // Use additive.code instead of code to avoid duplicate aliases
    ...
    if (!foundCodes.has(additive.code)) {
      ...
      if (additive.code === 'E322' && (
        matchesWordBoundary(text, 'lecithin') ||
  ...
  const regulatoryWarnings = detectedAdditives
    ...
    .map(a => {
      const restrictions = [];
      if (!a.permitted_EU) restrictions.push('EU');
      if (!a.permitted_NI) restrictions.push('Northern Ireland');
  ...
  let confidence = 0.8;
  if (eNumbers.length > 0) confidence += 0.1; // E-numbers are very reliable
  if (detectedAdditives.length === 0) confidence = 0.9; // High confidence in no additives
...
interface ProcessingScoreResult {
  totalScore: number;
  breakdown: { [category: string]: { count: number; score: number; details: string[] } };
}

function calculateProcessingScore(additives: AdditiveInfo[], ingredientsText: string): ProcessingScoreResult {
  const breakdown: { [category: string]: { count: number; score: number; details: string[] } } = {};
  let totalScore = 0;

  // Get unique additives by code to avoid double-counting
  const uniqueAdditives = additives.filter((additive, index, self) => 
  ...
  for (const [categoryName, rules] of Object.entries(PROCESSING_RULES.category_weights_notes)) {
    const categoryAdditives: AdditiveInfo[] = [];
    const ingredientMatches: string[] = [];
...
function determineGrade(totalScore: number, hasRedFlags: boolean): { grade: string; label: string } {
...
export { loadAdditiveDatabase, analyzeIngredientsForAdditives, calculateProcessingScore, determineGrade };