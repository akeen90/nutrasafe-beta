...
interface MicronutrientParseRequest {
  ingredientsText: string;
  ingredientsArray?: string[];
}

interface DetectedMicronutrient {
  nutrient: string;
  strength: 'strong' | 'moderate' | 'trace';
  source: string;
  confidence: number;
}
 ...
 */
export const parseMicronutrientsWithAI = onCall<MicronutrientParseRequest>(
  {
    cors: true,
    timeoutSeconds: 60,
    memory: '512MiB',
  ...
  async (request) => {
    ...
    if (!ingredientsText && !ingredientsArray) {
      throw new HttpsError('invalid-argument', 'Either ingredientsText or ingredientsArray must be provided');
    }
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to extract micronutrients using AI', { originalError: String(error) });
    }
  }
);
 ...
 */
export const parseMicronutrientsWithAICached = onCall<MicronutrientParseRequest>(
  {
    cors: true,
    timeoutSeconds: 60,
    memory: '512MiB',
...
function buildMicronutrientExtractionPrompt(ingredientsText: string): string {
  return `
...
async function callGeminiAPI(prompt: string, apiKey: string): Promise<DetectedMicronutrient[]> {
...
function parseAIResponse(responseText: string): DetectedMicronutrient[] {
  try {
    ...
    return parsed
      .filter((item: any) => item.nutrient && item.strength && item.source && typeof item.confidence === 'number' && item.confidence >= 0.80)
      .map((item: any) => ({
        nutrient: String(item.nutrient).toLowerCase(),
        strength: item.strength as 'strong' | 'moderate' | 'trace',
