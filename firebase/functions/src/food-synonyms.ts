...
// Type definitions for Algolia SDK v5
...
interface RegularSynonym extends BaseSynonym {
  type: "synonym";
  synonyms: string[];
}

interface OneWaySynonym extends BaseSynonym {
  type: "onewaysynonym";
  input: string;
  synonyms: string[];
}

interface AltCorrection extends BaseSynonym {
  type: "altcorrection1" | "altcorrection2";
  word: string;
  corrections: string[];
}

type FoodSynonym = RegularSynonym | OneWaySynonym | AltCorrection;

/**
 * A. Fast Food Menu Items (One-way synonyms)
 * When user searches "big mac", also search for McDonald's products
 */
const FAST_FOOD_MENU_SYNONYMS: OneWaySynonym[] = [
  // McDonald's
 ...
 */
const BRAND_SPELLING_SYNONYMS: RegularSynonym[] = [
 ...
 */
const UK_US_SPELLING_SYNONYMS: RegularSynonym[] = [
 ...
 */
const COMMON_MEAL_SYNONYMS: RegularSynonym[] = [
 ...
 */
const FOOD_TYPE_SYNONYMS: RegularSynonym[] = [
 ...
 */
const MISSPELLING_SYNONYMS: RegularSynonym[] = [
...
const ALL_SYNONYMS: FoodSynonym[] = [
 ...
 */
export const syncSynonymsToAlgolia = functions.https.onRequest({
  ...
  for (const indexName of ALL_INDICES) {
    try {
      await client.saveSynonyms({
        indexName,
        synonymHit: ALL_SYNONYMS as any[], // Cast to any for SDK compatibility
 ...
 */
export const getSynonymStats = functions.https.onRequest({
  secrets: [algoliaAdminKey],
  cors: true,
}, async (request, response) => {
  const client = algoliasearch(ALGOLIA_APP_ID, algoliaAdminKey.value());
 ...
 */
export const clearSynonymsFromAlgolia = functions.https.onRequest({
  secrets: [algoliaAdminKey],
  cors: true,
}, async (request, response) => {
