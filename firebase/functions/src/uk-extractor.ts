...
interface ExtractionRequest {
    url: string;
    barcode?: string;  // Optional - used for OpenFoodFacts fallback
    productName?: string;  // Optional - used for Tesco API search
    sourceType?: string;
...
interface ExtractionResult {
    ...
    extractionMethod: 'data_attributes' | 'embedded_json' | 'structured_data' | 'html_parsing' | 'puppeteer' | 'openfoodfacts' | 'tesco8_api' | 'failed';
    confidence: number;
    warnings: string[];
    sourceUrl: string;
    timestamp: string;
    ...
    debugInfo?: {
        htmlLength?: number;
        scriptTagsFound?: number;
        jsonLdFound?: boolean;
        embeddedJsonFound?: boolean;
        dataAttributesFound?: boolean;
        tablesFound?: number;
        nutritionTableFound?: boolean;
        textPatternsFound?: boolean;
...
function identifyAllergens(text: string): string[] {
    if (!text) return [];

    const allergens: string[] = [];
    const lowerText = text.toLowerCase();

    if (/\b(celery|celeriac)\b/.test(lowerText)) allergens.push('Celery');
    if (/\b(wheat|barley|rye|oats|spelt|kamut|gluten)\b/.test(lowerText)) allergens.push('Cereals containing gluten');
    if (/\b(crab|lobster|prawn|shrimp|crayfish|crustacean)\b/.test(lowerText)) allergens.push('Crustaceans');
    if (/\b(egg|eggs)\b/.test(lowerText)) allergens.push('Eggs');
    if (/\b(fish|cod|salmon|tuna|mackerel|anchov)\b/.test(lowerText)) allergens.push('Fish');
    if (/\b(lupin)\b/.test(lowerText)) allergens.push('Lupin');
    if (/\b(milk|dairy|cream|butter|cheese|lactose|whey|casein)\b/.test(lowerText)) allergens.push('Milk');
    if (/\b(mussel|oyster|squid|snail|clam|mollusc)\b/.test(lowerText)) allergens.push('Molluscs');
    if (/\b(mustard)\b/.test(lowerText)) allergens.push('Mustard');
    if (/\b(almond|hazelnut|walnut|cashew|pecan|pistachio|macadamia|nut|nuts)\b/.test(lowerText)) allergens.push('Nuts');
    if (/\b(peanut|groundnut)\b/.test(lowerText)) allergens.push('Peanuts');
    if (/\b(sesame)\b/.test(lowerText)) allergens.push('Sesame');
    if (/\b(soya|soy|soybean|soybeans)\b/.test(lowerText)) allergens.push('Soybeans');
    if (/\b(sulphite|sulfite|sulphur dioxide|so2)\b/.test(lowerText)) allergens.push('Sulphur dioxide');

    return [...new Set(allergens)];
}

function hasValidNutrition(nutrition: UKNutrition): boolean {
    const values = [
...
function cleanseProductName(name: string | undefined): string | undefined {
...
function cleanseBrandName(brand: string | undefined): string | undefined {
...
function cleanseIngredients(ingredients: string | undefined): string | undefined {
...
function cleanseNutritionValue(value: number | undefined, nutrientType: string): number | undefined {
...
function cleanseOpenFoodFactsData(product: ExtractedProduct): ExtractedProduct {
    const cleansed: ExtractedProduct = {
        ...product,
        ...
        allergens: product.allergens?.map(a =>
            a.replace(/^en:/, '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
        ) || [],
    };
...
function extractDataAttributes($: cheerio.CheerioAPI): ExtractedProduct | null {
    const dataSelectors = [
        '[data-product-info]',
        '[data-product]',
        '[data-initial-data]',
    ...
    for (const selector of dataSelectors) {
        ...
        let foundProduct: ExtractedProduct | null = null;

        elements.each((_, el): boolean | void => {
            if (foundProduct) return false;  // Break if found
...
function extractEmbeddedJson(html: string): { product: ExtractedProduct | null; method: string } {
...
function extractProductFromJson(json: any): ExtractedProduct | null {
...
function findProductInObject(obj: any, depth: number = 0): any {
...
function findNutritionInObject(obj: any, depth: number = 0): UKNutrition | null {
    ...
    if (keys.includes('per100g') || keys.includes('per_100g') || keys.includes('nutritionper100g')) {
        ...
        if (per100g) {
            ...
            if (hasValidNutrition(nutrition)) return nutrition;
        }
    }
    ...
    for (const key of Object.keys(obj)) {
        ...
        if (value && typeof value === 'object') {
            ...
            if (found && hasValidNutrition(found)) return found;
        }
    }
...
function extractNutritionValues(obj: any): UKNutrition {
    if (!obj || typeof obj !== 'object') return {};

    const nutrition: UKNutrition = {};
...
function extractJsonLd(html: string): ExtractedProduct | null {
    const $ = cheerio.load(html);
        ...
        try {
            ...
            if (data) {
                if (data['@type'] === 'Product') {
                    ...
                    productData = data['@graph'].find((g: any) => g['@type'] === 'Product');
                }
            }
        } catch (e) {
            // Invalid JSON
...
interface TableStructure {
    labelColumn: number;
    per100gColumn: number;
}

function findNutritionTable($: cheerio.CheerioAPI): { table: cheerio.Cheerio<any>; structure: TableStructure } | null {
...
function detectTableStructure($table: cheerio.Cheerio<any>, $: cheerio.CheerioAPI): TableStructure {
...
function extractFromIntelligentTable($: cheerio.CheerioAPI): ExtractedProduct | null {
    ...
    table.find('tr').each((_, row) => {
        ...
        if (label.includes('nutrient') || label.includes('typical values')) return;
    ...
    if (!hasValidNutrition(nutrition)) return null;
...
function extractFromTextPatterns($: cheerio.CheerioAPI): ExtractedProduct | null {
...
interface Tesco8NutritionItem {
    name: string;
    perComp: string;
    perServing: string;
}

async function tryTesco8Api(productName: string): Promise<{ product: ExtractedProduct | null; method: string; warnings: string[] }> {
    ...
    try {
        ...
        const searchResult: Tesco8SearchResult = searchResponse.data.data.products[0];
        ...
        const nutrition: UKNutrition = {};
        const nutritionItems: Tesco8NutritionItem[] = details.nutritionInfo || [];
...
async function tryOpenFoodFacts(barcode: string): Promise<{ product: ExtractedProduct | null; method: string; warnings: string[] }> {
    const warnings: string[] = [];
    ...
    try {
        ...
        if (response.data?.status === 1 && response.data.product) {
            ...
            // Build raw product
            const rawProduct: ExtractedProduct = {
...
export const extractUKProductData = functions
    .runWith({ timeoutSeconds: 60, memory: '512MB' })
    .https.onCall(async (data: ExtractionRequest, _context): Promise<ExtractionResult> => {
        const { url, barcode } = data;
        const timestamp = new Date().toISOString();
        const warnings: string[] = [];
        const debugInfo: ExtractionResult['debugInfo'] = { tiersAttempted: [] };
        ...
        try {
            ...
            let extractedData: ExtractedProduct | null = null;
            let extractionMethod: ExtractionResult['extractionMethod'] = 'failed';
            let confidence = 0;
            ...
            if (extractedData && hasValidNutrition(extractedData.nutrition)) {
                ...
                if (!n.energyKcal) warnings.push('Missing energy (kcal)');
                if (n.fat === undefined) warnings.push('Missing fat');
                if (n.carbohydrate === undefined) warnings.push('Missing carbohydrate');
                if (n.protein === undefined) warnings.push('Missing protein');
                if (n.salt === undefined) warnings.push('Missing salt');
...
export const extractWithPuppeteer = functions
    .runWith({ timeoutSeconds: 120, memory: '2GB' })
    .https.onCall(async (data: ExtractionRequest, _context): Promise<ExtractionResult> => {
        const { url, barcode } = data;
        const timestamp = new Date().toISOString();
        const warnings: string[] = [];
        const debugInfo: ExtractionResult['debugInfo'] = { tiersAttempted: ['puppeteer'] };
        ...
        try {
            ...
            let extractedData: ExtractedProduct | null = null;
            let extractionMethod: ExtractionResult['extractionMethod'] = 'puppeteer';
            let confidence = 0;
...
export const lookupTescoProduct = functions
    .runWith({ timeoutSeconds: 30, memory: '256MB' })
    .https.onCall(async (data: { productName: string }, _context): Promise<ExtractionResult> => {
        const { productName } = data;
        const timestamp = new Date().toISOString();
        const warnings: string[] = [];
        const debugInfo: ExtractionResult['debugInfo'] = { tiersAttempted: ['tesco8_api_direct'] };
