...
export interface UKProduct {
  names: string[]; // Alternative names for matching
  brands: string[];
  servingSize: {
    amount: number;
    unit: string;
...
export const UK_PRODUCT_DATABASE: UKProduct[] = [
  {
...
export function findUKProduct(name: string, brand?: string): UKProduct | null {
  const normalizedName = name.toLowerCase().trim();
  const normalizedBrand = brand?.toLowerCase().trim();
  
  for (const product of UK_PRODUCT_DATABASE) {
    // Check if name matches any of the product names
    const nameMatch = product.names.some(productName => 
      normalizedName.includes(productName.toLowerCase()) ||
      productName.toLowerCase().includes(normalizedName)
    );
    
    // Check if brand matches (if provided)
    const brandMatch = !normalizedBrand || product.brands.some(productBrand =>
      normalizedBrand.includes(productBrand.toLowerCase()) ||
...
export function calculateNutritionPerServing(product: UKProduct): any {
