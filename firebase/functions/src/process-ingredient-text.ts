...
interface ProcessIngredientTextRequest {
  textChunks: string[];
  sessionId: string;
...
interface ProcessIngredientTextResponse {
  success: boolean;
...
export const processIngredientText = functions.https.onRequest(async (req, res) => {
  corsHandler(req, res, async () => {
    try {
      ...
      const { textChunks, sessionId, scanType, finalProcess }: ProcessIngredientTextRequest = req.body;
      ...
      if (!combinedText) {
        res.json({
          ...
          confidence: 0,
          sessionId
        } as ProcessIngredientTextResponse);
        return;
      }
      ...
      const processResponse: ProcessIngredientTextResponse = {
        success: true,
        processedText: text,
        structuredData,
...
function createIngredientsPrompt(text: string, finalProcess: boolean): string {
  const basePrompt = `
...
function createNutritionPrompt(text: string, finalProcess: boolean): string {
  const basePrompt = `
...
function parseGeminiResponse(text: string, scanType: string): any {
  try {
...
function parseNutritionFromText(text: string): any {
  // Fallback parsing for nutrition text
  const nutrition: any = {};
  const lines = text.split('\n');

  const nutritionMapping: { [key: string]: string[] } = {
