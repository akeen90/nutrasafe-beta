...
export const searchFoodsWithMicronutrients = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
...
async function searchVerifiedFoods(query: string): Promise<any[]> {
  try {
...
async function searchUKDatabase(query: string): Promise<any[]> {
  try {
...
async function enhanceWithMicronutrients(verifiedResults: any[], ukResults: any[]): Promise<any[]> {
  ...
  const enhanced = await Promise.all(uniqueResults.map(async (food) => {
    ...
    return {
      ...
      micronutrients: micronutrients ? {
        ...micronutrients,
        dataSource: micronutrients.dataSource,
        confidenceScore: micronutrients.confidenceScore
      } : null
    };
  }));
  ...
  return enhanced.sort((a, b) => {
    ...
    if (a.micronutrients && !b.micronutrients) return -1;
    if (!a.micronutrients && b.micronutrients) return 1;
...
async function getMicronutrientsForFood(food: any): Promise<any> {
  try {
...
async function performSimpleRecipeEstimation(food: any): Promise<any> {
  ...
  if (typeof food.ingredients === 'string') {
    ...
    ingredients = food.ingredients.split(',').map((ingredient: string) => ingredient.trim().toLowerCase());
  ...
  const ingredientNutrientSources: { [key: string]: string[] } = {
...
async function getCategoryBasedMicronutrients(food: any): Promise<any> {
