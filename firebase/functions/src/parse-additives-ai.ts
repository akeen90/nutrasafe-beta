...
interface AdditiveParseRequest {
  ingredientsText: string;
  ingredientsArray?: string[];
}

interface DetectedAdditive {
  name: string;
  eNumber: string;
  category: string;
  safety: 'safe' | 'neutral' | 'caution' | 'avoid';
  confidence: number;
}

export const parseAdditivesWithAI = onCall<AdditiveParseRequest>(
  {
    cors: true,
    timeoutSeconds: 60,
    memory: '512MiB',
  ...
  async (request) => {
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to extract additives using AI', { originalError: String(error) });
    }
  }
);

export const parseAdditivesWithAICached = onCall<AdditiveParseRequest>(
  {
    cors: true,
    timeoutSeconds: 60,
    memory: '512MiB',
...
function buildAdditiveExtractionPrompt(ingredientsText: string): string {
  return `
...
async function callGeminiAPI(prompt: string, apiKey: string): Promise<DetectedAdditive[]> {
...
function parseAIResponse(responseText: string): DetectedAdditive[] {
  try {
    ...
    return parsed
      .filter((item: any) => item.name && item.eNumber && item.category && item.safety && typeof item.confidence === 'number' && item.confidence >= 0.75)
      .map((item: any) => ({
        name: String(item.name),
        eNumber: String(item.eNumber).toUpperCase(),
        category: String(item.category).toLowerCase(),
        safety: item.safety as 'safe' | 'neutral' | 'caution' | 'avoid',
