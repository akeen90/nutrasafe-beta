...
interface ServingContext {
  size: number; // multiplier (1x, 3x, etc.)
  description: string; // "1 biscuit", "small handful", "packet"
  likelihood: 'low' | 'medium' | 'high'; // how likely someone is to eat this amount
}

interface ContextualScore {
  score: string; // A, B, C, etc.
  message: string;
  metabolicImpact: string;
  balanceTip?: string;
}

export const getContextualNutritionScore = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
...
function getConsumptionContexts(foodName: string, servingSizeGrams: number): ServingContext[] {
