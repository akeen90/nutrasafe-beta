...
import { findUKProduct, calculateNutritionPerServing } from './uk-product-database';
...
export const analyzeAndCleanFoods = functions.runWith({
...
async function analyzeChunkWithAI(foods: any[], apiKey: string, retryCount = 0): Promise<{results: CleaningResult[], cost: number}> {
  const maxRetries = 3;
...
IMPORTANT: Return the data as a JSON object with a "foods" array containing the processed food objects. Example: {"foods": [...]}`
    ...
    const results: CleaningResult[] = aiResults.map((aiResult, index) => {
      try {
        ...
        const ukProduct = findUKProduct(foodName, brandName);
        ...
        return {
          ...
          nutritionData: (() => {
            ...
            function parseServingSize(serving: string): number | null {
              if (!serving || serving === '100g' || serving === '100g serving') return 100;
            ...
            if (ukProduct) {
              ...
              per100g.perServing = calculateNutritionPerServing(ukProduct);
...
export const getCleansedFoods = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  ...
  try {
    const { 
      filter = 'all', 
      limit = 1000,
      sortBy = 'recent', // recent, name, brand, severity, calories
      search = '',
      brand = '',
      hasBarcode = '',
      hasIngredients = '',
    ...
    for (const collectionName of collections) {
      try {
        ...
        const snapshot = await query.limit(Math.ceil(parseInt(limit as string) / collections.length)).get();
    ...
    if (search && typeof search === 'string') {
      ...
      filteredFoods = filteredFoods.filter(food => 
        (food.name || food.foodName || '').toLowerCase().includes(searchTerm) ||
    ...
    filteredFoods.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'severity':
          ...
          const aSev = severityOrder[a.aiAnalysis?.severity as keyof typeof severityOrder] || 1;
          const bSev = severityOrder[b.aiAnalysis?.severity as keyof typeof severityOrder] || 1;
          return bSev - aSev;
    ...
    filteredFoods.slice(0, 5).forEach((food: any, index) => {
...
export const exportCleansedFoods = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
...
export const deleteCleansedFood = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
...
export const updateCleansedFood = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
...
export const emergencyCleanCleansedFoods = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
...
export const updateCleansedFoodStatus = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
