...
export const getOverviewStats = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
...
// Get analytics data
export const getAnalyticsData = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  ...
  try {
    ...
    const nutritionGrades: { [key: string]: number } = {};
...
function calculateAverageNutritionScore(docs: admin.firestore.QueryDocumentSnapshot[]): number {
  let totalScore = 0;
  let scoredFoods = 0;
