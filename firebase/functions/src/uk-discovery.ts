...
// ============================================================
// APPROVED UK SOURCES (WHITELIST)
// Only these domains will be returned in search results
// ============================================================

interface ApprovedSource {
    domain: string;
    name: string;
    type: 'supermarket' | 'manufacturer' | 'retailer';
    productPattern?: RegExp;
    priority: number; // Lower = higher priority
}

const UK_APPROVED_SOURCES: ApprovedSource[] = [
...
interface DiscoveredSource {
    ...
    domain: string;
    sourceName: string;
    sourceType: 'supermarket' | 'manufacturer' | 'retailer' | 'unknown';
    title: string;
    snippet: string;
...
const BARCODE_PREFIXES: { prefix: string; country: string; isUK: boolean }[] = [
...
function getBarcodeCountry(barcode: string): { country: string; isUK: boolean } | null {
    ...
    for (const len of [3, 2]) {
        ...
        if (match) return { country: match.country, isUK: match.isUK };
    }
...
function findApprovedSource(url: string): ApprovedSource | null {
    const domain = extractDomain(url);
    return UK_APPROVED_SOURCES.find(source =>
...
function calculateConfidence(
    ...
    if (source.priority === 1) confidence += 20;
    else if (source.priority === 2) confidence += 10;
...
function isProductPage(url: string, source: ApprovedSource): boolean {
...
export const discoverUKProductPage = functions.https.onCall(
    async (data: DiscoveryRequest, context): Promise<DiscoveryResult> => {
        ...
        let barcodeCountry: { country: string; isUK: boolean } | null = null;
        if (hasBarcode) {
        ...
        for (const strategy of searchStrategies) {
            try {
                ...
                // Process results
                const foundSources: DiscoveredSource[] = [];
...
export const generateManualSearchLinks = functions.https.onCall(
    async (data: DiscoveryRequest, context): Promise<{
        success: boolean;
        googleQuery: string;
        supermarketLinks: { name: string; url: string; domain: string }[];
    }> => {
        const { foodName, barcode, brand } = data;
