...
export const updateVerifiedFood = functions.runWith({ secrets: [algoliaAdminKey] }).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (servingSizeG !== undefined) updateData.servingSizeG = servingSizeG;
    if (servingUnit !== undefined) updateData.servingUnit = servingUnit;
    if (isPerUnit !== undefined) updateData.isPerUnit = isPerUnit;
    ...
    if (ALGOLIA_ONLY_INDICES.includes(targetCollection)) {
      ...
      if (algoliaKey) {
        ...
        const collectionToIndex: Record<string, string> = {
...
export const deleteFoodFromAlgolia = functions.runWith({
  ...
  try {
    ...
    if (barcode) {
      ...
      try {
        ...
        const searchResponse = await axios.get(`https://${TESCO8_HOST}/product-search-by-keyword`, {
        ...
        if (searchResponse.data?.success && searchResponse.data?.products?.length > 0) {
          ...
          const matchingProduct = searchResponse.data.products.find((p: any) => p.gtin === barcode || p.ean === barcode);
          if (matchingProduct) {
            // Get full product details
            const detailsResponse = await axios.get(`https://${TESCO8_HOST}/product-details`, {
    ...
    try {
      ...
      if (updates) {
        ...
        if (updates.nutrition) {
          ...
          if (n.calories !== undefined && n.calories !== null) updateObj.calories = n.calories;
          if (n.protein !== undefined && n.protein !== null) updateObj.protein = n.protein;
          if (n.carbs !== undefined && n.carbs !== null) {
            updateObj.carbs = n.carbs;
            updateObj.carbohydrates = n.carbs;
          }
          if (n.fat !== undefined && n.fat !== null) updateObj.fat = n.fat;
          ...
          if (n.salt !== undefined && n.salt !== null) updateObj.salt = n.salt;
          if (n.sodium !== undefined && n.sodium !== null) updateObj.sodium = n.sodium;
 ...
 */
export const deleteFoodComprehensive = functions.runWith({
  secrets: [algoliaAdminKey],
  timeoutSeconds: 60,
  memory: '256MB',
}).https.onRequest(async (req, res) => {
    ...
    if (barcode && deleteFromAllIndices) {
      ...
      for (const idx of ALL_ALGOLIA_INDICES) {
        ...
        try {
          ...
          const matchingFoods = (searchResult.hits || []).filter((hit: any) => {
            return hit.barcode === barcode || hit.gtin === barcode;
          });

          for (const hit of matchingFoods) {
            const objId = (hit as any).objectID;
            try {
